...
class DbProvider {
  ...
  Future<int> add(Todo todo) async => _database.insert(
        TABLE_NAME,
        todo.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace,
      );

  Future<int> remove(Todo todo) async => _database.delete(
        TABLE_NAME,
        where: '$ID = ?',
        whereArgs: [todo.id],
      );

  Future<int> update(Todo todo) async => _database.update(
        TABLE_NAME,
        todo.toMap(),
        where: '$ID = ?',
        whereArgs: [todo.id],
  ...
  Future<void> _initDataBase() async {
    if (_database == null) {
      _database = await openDatabase('$_dbKey\_$DB_NAME', version: 1,
