...
class _ReporterPageState extends State<ReporterPage> {
  TodoList _todoList;
  int _finishedTodoCount = 64;
  int _handlingTodoCount = 32;
  int _delayedTodoCount = 12;

  List<Todo> _todosOfThisMonth = [];
  List<Todo> _todosToShow = [];

  TodoStatus _showedStatus;
  int currentMonth;

  @override
  void initState() {
    super.initState();
    _todoList = TodoList(generateTodos(10));
    currentMonth = 1;
  }

  void _reset() {
    _finishedTodoCount = 0;
    _handlingTodoCount = 0;
    _delayedTodoCount = 0;
  ...
  void _compute(TodoList todoList, int month) {
    ...
    todoList.list.forEach((todo) {
      if (todo.date != null && todo.date.month == month) {
        ...
        if (status == TodoStatus.handling) {
          _handlingTodoCount += 1;
        }
    ...
    _computeByTodoStatus(null);
  }

  void _computeByTodoStatus(TodoStatus status) {
    if (_showedStatus == status && _todosToShow != null) {
      return;
    }
    if (status == null) {
      _todosToShow = _todosOfThisMonth;
    } else {
      _todosToShow =
          _todosOfThisMonth.where((t) => t.status == status).toList();
    }
  ...
  Widget build(BuildContext context) {
    _compute(_todoList, currentMonth);
  ...
  Widget _buildAllTodoStatusArea() {
    return Row(
      ...
      children: <Widget>[
        ...
        _buildTodoStatusView(TodoStatus.handling, _handlingTodoCount),
  ...
  Widget _buildColouredStripeArea() {
    int sum = _finishedTodoCount + _handlingTodoCount + _delayedTodoCount;
    return Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            _buildColouredStripe(
                _handlingTodoCount / sum, const Color(0xff8c88ff)),
            _buildColouredStripe(
  ...
  Widget _buildTodoStatusView(TodoStatus status, int count) {
    ...
    return Expanded(
      ...
      child: GestureDetector(
        child: Container(
          color:
              _showedStatus == status ? Colors.lightGreen : Colors.transparent,
          child: Column(
            children: <Widget>[
        ...
        onTap: () {
          _computeByTodoStatus(status == _showedStatus ? null : status);
        },
      ),
    );
  }
